...
public class EditRedo extends GPAction {

	protected Vector menuItems = new Vector();

	public void actionPerformed(ActionEvent e) {
		try {
			getCurrentGPDocument().getGraphUndoManager() .redo(getCurrentGPDocument().getGraph().getGraphLayoutCache());
	...
	public void update() {
		...
		while(e_num.hasMoreElements()){
			...
			if (getCurrentGPDocument() != null&&
				getCurrentGPDocument().getGraphUndoManager().canRedo(getCurrentGraphLayoutCache())) {
				setEnabled(true);
				item.setText(
					getCurrentGPDocument().getGraphUndoManager().getRedoPresentationName());
			} else {
				setEnabled(false);
