 ...
 */
public class FilePageFormat extends GPAction {
			...
			try {
				PageFormat f =
					PrinterJob.getPrinterJob().pageDialog(
							getCurrentGPDocument().getPageFormat());
				getCurrentGPDocument().setPageFormat(f);
				mdiContainer.getCurrentDocument().updatePageFormat();
			} catch (Exception ex) {
